...
class ArticleList extends Component {
      ...
      <div className="artcleList admin-container">
        <div className="category-title admin-title">
          ...
          <div className="btn-group">
            <Button type="primary"><Icon type="plus-circle-o" />新增文章</Button>
          </div>
        </div>
  ...
  setDefaultPageSize () {
    ...
    this.setState({
      defaultPageSize: Math.ceil(leftHeight / 35) - 1
    })
  }
  // 渲染文章列表
  renderArticleList () {
    ...
    return articleList.map(item => (
      <li key={item.id}>
        ...
        <div className="article-control"><Icon type="edit" /><Icon type="paper-clip" /><Icon type="close-circle" /></div>
      </li>
    ))
  }
  // 显示alert
